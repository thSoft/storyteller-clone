...
export const love: Scene = {
  ...
  outcomeLogic: (state, assigned) => {
    ...
    if (state.getState(lover1.id, "sex") === state.getState(lover2.id, "sex")) {
      state.setGlobalState("event", `${lover1.name} and ${lover2.name} are of the same sex.`);
      return;
    }
    ...
    if (engaged !== undefined && engaged2 !== undefined && engaged.id !== engaged2.id) {
      state.setGlobalState(
        "event",
        `Both ${engaged.name} and ${engaged2.name} were already in love with someone else.`
      );
      return;
    }
    if (engaged !== undefined && rejected !== undefined) {
      ...
      state.setGlobalState("event", `${engaged.name} rejected ${rejected.name} because of ${theThirdOne.name}.`);
      if (state.getState(rejected.id, "spiteful")) {
        state.addRelation(rejected.id, "angryAt", theThirdOne.id);
        state.setGlobalState("event", `${rejected.name} got angry at ${theThirdOne.name}.`);
      } else {
        state.setState(rejected.id, "heartbroken", true);
        state.setGlobalState("event", `${rejected.name} was heartbroken.`);
      }
      return;
    }
    state.addRelation(lover1.id, "loves", lover2.id);
    state.addRelation(lover2.id, "loves", lover1.id);
    state.setGlobalState("event", `${lover1.name} and ${lover2.name} fell in love.`);
  },
};
